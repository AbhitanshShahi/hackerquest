...
document
  ...
  .forEach((b) => b.addEventListener("click", () => showPage("page-home")));
document
  .querySelectorAll(
    ".category-card-food, .category-card-medical, .category-card-education, .category-card-house"
  )
  .forEach((card) => {
    card.addEventListener("click", () => {
      const pageId = card.dataset.page;
...
                <span>By: ${data.userEmail.split("@")[0]}</span>
            ...
            <a href="mailto:${
              data.userEmail
            }" class="btn-contact" title="Contact via Email">
...
                <span>Qty: ${data.quantity}</span>
            ...
            <a href="mailto:${data.userEmail}" class="btn-contact" title="Contact via Email">
                <i class="ph ph-envelope-simple"></i>
            </a>
        </div>`;
  return card;
...
async function scanImageWithGemini(base64ImageData, mimeType = "image/jpeg") {
  ...
  try {
    ...
    const result = await response.json();
    console.log("Gemini result:", result); // Safely parse text
    ...
    const scanResults = document.getElementById("scan-results");
    if (scanResults) scanResults.style.display = "block";
    if (identifiedItemsText)
      identifiedItemsText.textContent = "❌ Scan failed. Please try again.";
  }
}
  ...
  populateFormBtn.addEventListener("click", () => {
    if (identifiedItemsText && homeFormDescription) {
      ...
      if (items) {
        homeFormDescription.value = items;
        alert(
          "✅ Description field filled. Please complete the rest of the form."
        );
      }
    }
  });
